...
- DEV Hosting、Firestore Rules/indexes、二十五個 Node.js 22 Functions 已部署；網址：`https://bini-transient-dev.web.app`。
...
DEV Functions                  15/15 listed, asia-east1, nodejs22; paymentCreate ACTIVE, 512 MiB
...
DEV Hosting                    index-Bcv9-lw7.js live; bundle contains paymentManualCreate, paymentRefund and 手動例外收款 markers
...
3. room overview 即時讀取、check-in、extend、checkout、active-stay 一般收款、追加式退款與手動例外收款已完成；接續完成訂金調整／刪除／日結、房務／維修其餘 server-authoritative operations，再依序完成 gantt → bookings 多時段與前置 quote → reports/costs/holidays/properties/audit。每個 domain 要有 role matrix、payload schema、idempotency/conflict tests。
