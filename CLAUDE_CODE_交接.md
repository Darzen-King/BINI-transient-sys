...
- DEV Hosting、Firestore Rules/indexes、二十六個 Node.js 22 Functions 已部署；網址：`https://bini-transient-dev.web.app`。
...
DEV Hosting                    index-CjoVHka-.js live; bundle contains cashierClose, paymentManualCreate and 執行今日日結 markers
...
3. room overview 即時讀取、check-in、extend、checkout、active-stay 一般收款、追加式退款、手動例外收款與日結已完成；接續完成訂金調整／刪除／CSV、房務／維修其餘 server-authoritative operations，再依序完成 gantt → bookings 多時段與前置 quote → reports/costs/holidays/properties/audit。每個 domain 要有 role matrix、payload schema、idempotency/conflict tests。
