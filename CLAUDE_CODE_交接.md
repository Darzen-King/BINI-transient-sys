...
3. room overview 即時讀取、check-in 與 extend 已完成；接續完成 room status／payment／checkout 的 server-authoritative operations，再依序完成 gantt → bookings 多時段與前置 quote → stays/transfer/monthly → payments/cashier → housekeeping/maintenance → reports/costs/holidays/properties/audit。每個 domain 要有 role matrix、payload schema、idempotency/conflict tests。
