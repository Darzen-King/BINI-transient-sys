...
## 程式內自動更新（v3.9.4 新增，僅系統管理員）

- 位置：**雲端備份**頁上方「🔄 系統更新」卡片。
- **檢查更新**：比對目前版本與 GitHub 最新版。
- **立即更新**：按下後系統會依序自動完成——
  1. **先備份到雲端**（Dropbox）；**備份失敗就中止，絕不動到程式**。
  2. 下載新版、覆蓋程式碼（**保留資料庫 / 本機備份 / log，不會刪任何資料**）。
  3. 重新安裝相依套件、**自動重新啟動**。
- 不必再重新下載 zip 手動安裝。
- 首次要能用這顆按鈕，需先手動安裝一次 **v3.9.4**；之後的版本都可一鍵更新。
...
```
v3.9.4 | FastAPI 0.111.0 | SQLite 3 | Python 3.12 | pywebview 6.2.1
```
