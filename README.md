...
### 甘特圖（/gantt）— v3.8.3 改為 2 週
- 顯示**今天起 2 週（14 天）**的所有房間使用狀況
- 橫軸：日期 + 每 4 小時刻度 | 縱軸：房號
- 🟢 入住中 | 🟡 預約 | 🔴 維修 | ⬛ 月租 | □ 空房
...
### 月租套房（v3.9.2 新增續租 / v3.9.3 收入認列）
- 房間管理可對月租套房 **🔁 續租一個月**：租期自動往後延一個月（押金結轉不重複收取，續租租金計入報表營收）
- 「下一筆預約」由系統即時帶出，**唯讀**不可手動修改
- **月租租金以「實際收款日」認列營收**（建立或續租當下＝收租金當月），不受起租日影響

### 衝突偵測（v3.8.3 強化）
- **新增預約**與**入住登記**都會檢查時段衝突，任一方向重疊即阻擋
- 同時比對「既有預約」與「目前入住中的旅客」兩種來源（先前入住登記漏做此檢查，且預約檢查未涵蓋在住旅客）
- 旅客憑自己的預約辦理入住不會被誤判為衝突
...
```
v3.9.3 | FastAPI 0.111.0 | SQLite 3 | Python 3.12 | pywebview 6.2.1
```
