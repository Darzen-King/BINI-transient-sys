...
/**
 * Projects that run the Dropbox backup. A project not listed never declares the Dropbox secrets — the Firebase
 * CLI would otherwise stop the whole deploy to ask for their values (PROD until they are set) — and only one
 ...
 */
function createDropboxV3Backup() {
...
export const dropboxV3Backup = dropboxBackupEnabled() ? createDropboxV3Backup() : undefined;
