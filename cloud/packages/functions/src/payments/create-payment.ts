...
function fingerprint(input: PaymentCreateInput): string { return createHash('sha256').update(JSON.stringify({ operationType: 'payment.create', ...input })).digest('hex'); }
...
/** Creates one normal payment for a currently active stay. Client writes never create payments directly. */
export const paymentCreate = onCall(options, async (request): Promise<PaymentCreateResult> => {
  ...
  return database.runTransaction(async (transaction) => {
    ...
    transaction.create(database.doc(`${root}/payments/${paymentId}`), { schemaVersion: 4, version: 1, propertyId: input.propertyId, bookingId: typeof stay.bookingId === 'string' ? stay.bookingId : null, stayId: input.stayId, roomId, guestName: text(stay, 'guestName', stayLabel), paymentType: input.paymentType, amountNts: input.amountNts, deposit: false, refund: false, status: 'paid', note: input.note ?? null, createdByUid: actorUid, createdAt: now });
    ...
    transaction.create(database.doc(`${root}/auditLogs/payment-create-${input.operationId}`), { actorUid, action: 'payment.create', targetId: paymentId, targetType: 'payment', details: { operationId: input.operationId, stayId: input.stayId, roomId, amountNts: input.amountNts, paymentType: input.paymentType }, createdAt: now });
    return result;
  });
});
