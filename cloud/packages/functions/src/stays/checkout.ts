...
export const stayCheckout = onCall(options, async (request): Promise<StayCheckoutResult> => {
  ...
  return database.runTransaction(async (transaction) => {
    ...
    const checkInAt = text(stay, 'checkInAt', stayLabel); const checkOutAt = text(stay, 'checkOutAt', stayLabel); const originalCheckOutAt = typeof stay.originalCheckOutAt === 'string' ? stay.originalCheckOutAt : checkOutAt; const now = new Date().toISOString(); const minutesSinceCheckIn = Date.parse(now) - Date.parse(checkInAt); const freeCancel = minutesSinceCheckIn >= 0 && minutesSinceCheckIn <= 15 * 60 * 1_000;
    ...
    if (!freeCancel) { const quote = quoteStayCheckoutOverdue(checkInAt, originalCheckOutAt, existingExtensionFee, now, holidays(holidaySnapshot.docs)); systemOverdueFeeNts = quote.systemOverdueFeeNts; appliedOverdueFeeNts = input.overdueFeeOverrideNts ?? systemOverdueFeeNts; extensionFeeNts += appliedOverdueFeeNts; }
