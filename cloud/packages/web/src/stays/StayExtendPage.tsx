...
export function StayExtendPage({ session, gateway, staysGateway, holidayGateway, onBack }: {
  ...
  const preview = useMemo(() => {
    if (!selectedStay || !calendar || !Number.isFinite(extensionHours) || extensionHours < 0.5) return null;
    try {
  ...
  return <SectionCard hint={text('伺服器交易驗證', 'Server-authoritative transaction')} title={text('延住處理', 'Extend stay')}>
    ...
    <form className="booking-create-form" onSubmit={(event) => void submit(event)}>
      <div className="booking-create-grid">
        ...
        <Field label={text('延住時數', 'Extension hours')}><input disabled={unavailable || !selectedStay} max="168" min="0.5" onChange={(event) => setExtensionHours(Number(event.target.value) || 0)} required step="0.5" type="number" value={extensionHours} /></Field>
      </div>
