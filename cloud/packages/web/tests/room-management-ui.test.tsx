...
const rooms: RoomManagementItem[] = [
  { roomId: '201', status: '可入住', note: 'Near lift', maintenanceNote: null, maintenanceDueDate: null, monthly: null },
  { roomId: '206', status: '月租套房', note: null, maintenanceNote: null, maintenanceDueDate: null, monthly: { rentalId: 'MR-live', tenantName: 'Carlos', tenantPhone: null, startDate: '2026-09-01', endDate: '2026-10-01', depositNts: 2_000, rentNts: 9_000, paymentType: 'cash', note: null } },
];
afterEach(cleanup);
...
describe('room management UI', () => {
  it('opens mobile-compatible room details and uses callable operations for monthly actions', async () => {
    ...
    const gateway: RoomManagementGateway = { subscribe(_propertyId, onValue) { queueMicrotask(() => onValue(rooms)); return () => undefined; }, update: vi.fn(), createMonthly: vi.fn(), renewMonthly, checkoutMonthly: vi.fn() };
