...
| 5 | 付款管理 `payments.html` | `/payments`、新增／刪除付款、選在住房客或手動輸入、日結、CSV 匯出 | `payments` 即時讀取、`paymentCreate`／`paymentRefund`／`paymentManualCreate`／`cashierClose` callable；後續 `payment.delete` 與伺服器匯出 | 已接當日摘要、付款紀錄、在住房一般收款、追加式退款、手動例外收款與日結；帳務維護待補 | 同一 responsive 收款表單、摘要卡、付款卡、退款／手動收款／日結 dialog | **部分完成：MFA＋`payments` 權限的一般收款、追加式退款、手動例外收款、即時紀錄／摘要、transaction／audit／operation replay 已接；日結限 manager/admin，以 server 台北日期彙總並鎖定每日 session。訂金調整、刪除與 CSV 待實作** |
