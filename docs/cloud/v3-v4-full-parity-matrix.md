...
| 19 | 成本紀錄 `costs.html` | `/costs`、新增／修改／刪除、分類／付款方式／定期／收據、損益 | `costEntries` 即時 listener；`costCreate`／`costUpdate`／`costArchive` callable；P&L／CSV 待報表投影 | 已接月成本／筆數／分類摘要、月份／分類篩選、完整欄位新增／修改與明細；實體刪除改為 audit 封存 | 單欄表單、成本卡、修改／封存 ResponsiveDialog；與桌機共用 contract／gateway | **部分完成：MFA＋`costs` 分頁權限＋property admin 的新增／修改／封存、版本衝突、UUID replay、即時清單與 append-only audit 已接；營收、淨利、報表同步與 CSV 待實作** |
