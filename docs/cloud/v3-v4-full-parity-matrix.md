...
| 6 | 預約管理 `bookings.html` | `/bookings`、取消、No-show、編輯、即將入住提醒 | `bookings`；`bookingCancel`；可查詢狀態與時間區間 | 保留計數標籤、排序表格與列操作 | 狀態 tabs、搜尋、預約卡與 action sheet | **部分完成：有效預約即時讀取、排序、搜尋、明細與單筆取消已接；No-show、編輯、提醒待實作** |
