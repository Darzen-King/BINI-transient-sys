...
| 15 | 統計報表 `reports.html` | `/reports`、日期篩選、住宿／月租統計、CSV | `rooms`／`bookings`／`stays`／`stayLogs`／`monthlyRentals` 即時 projection；`reportExportCsv` 伺服器重算與 audit；admin 加讀 `costEntries` | 已接日期篩選、v3 區間營收口徑、KPI、每日／房間明細與 CSV | KPI 卡、清單明細與系統下載 CSV | **部分完成：manager/admin 即時營運報表與 audited CSV、admin 成本／損益已接；單館別部署故未提供館別切換，付款日摘要、圖表與完整 v3 報表分區待補** |
