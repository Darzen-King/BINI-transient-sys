...
| 5 | 付款管理 `payments.html` | `/payments`、新增／刪除付款、選在住房客或手動輸入、日結、CSV 匯出 | `payments` 即時讀取、`paymentCreate`／`paymentRefund`／`paymentManualCreate`／`paymentVoid`／`cashierClose`／`paymentExportCsv` callable | 已接當日摘要、付款紀錄、在住房一般收款、追加式退款、手動例外收款、管理員安全作廢、日結與伺服器帳務 CSV；訂金調整待補 | 同一 responsive 收款表單、摘要卡、付款卡、退款／作廢／手動收款／日結 dialog 與日期區間 CSV 匯出 | **部分完成：MFA＋`payments` 權限的一般收款、追加式退款、手動例外收款、即時紀錄／摘要、transaction／audit／operation replay 已接；日結限 manager/admin，以 server 台北日期彙總並鎖定每日 session。admin 作廢會保留原付款與原因／audit、從即時彙總排除，且已有退款或已日結時拒絕。付款 CSV 以台北日期區間、v3 欄位順序、UTF-8 BOM、operation replay 與 audit 完成；訂金調整待實作** |
