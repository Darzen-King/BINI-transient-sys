...
| 14 | 維修管理 `maintenance.html` | `/maintenance`、房間維修狀態、排程建立／刪除／完成 | `maintenanceSchedules` 即時讀取、`maintenanceScheduleCreate`；後續 update/delete/complete | 已接排程清單、建立與預約衝突檢查 | 單欄排程表單與觸控清單 | **部分完成：MFA＋`maintenance` 權限、建立、有效預約衝突拒絕、audit／operation replay 已接；解除維修、進度備註、排程完成／刪除與篩選待實作** |
