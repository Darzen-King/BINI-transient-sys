...
| 12 | 房間管理 `room_management.html` | `/room-management`、房態／備註、月租建立／續租／退租、換房、候選房 | `roomManagementUpdate`、`monthlyRentalCreate/renew/checkout`；狀態與月租 transaction | 即時房間卡保留房態、備註、維修與月租明細；在住房仍必須走入住／退房流程 | 點房卡開啟明細與操作 sheet；不壓縮桌機資訊 | **部分完成：MFA＋`room_management` 權限、備註／維修房態、月租建立／續租／退租、付款／audit／operation replay 已接；換房與候選房 wizard 待實作** |
