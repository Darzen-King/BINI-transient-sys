...
| 5 | 付款管理 `payments.html` | `/payments`、新增／刪除付款、選在住房客或手動輸入、日結、CSV 匯出 | `payments` 即時讀取、`paymentCreate`／`paymentRefund`／`paymentManualCreate` callable；後續 `cashierSessions`、`payment.delete`、`cashier.close` 與伺服器匯出 | 已接當日摘要、付款紀錄、在住房一般收款、追加式退款與手動例外收款；帳務維護待補 | 同一 responsive 收款表單、摘要卡、付款卡、退款 dialog 與手動收款 dialog | **部分完成：MFA＋`payments` 權限的一般收款、追加式退款、手動例外收款、即時紀錄／摘要、transaction／audit／operation replay 已接；退款保留原付款並在伺服器重新檢查可退餘額，手動收款強制旅客／原因並驗證選填房號。訂金調整、刪除、日結與 CSV 待實作** |
