...
| 16 | 審計軌跡 `audit.html` | `/admin/audit`、操作／日期／關鍵字篩選、唯讀 | `auditLogs` property-scoped listener；v3 `activity_logs` 與 v4 append-only audit；client write 永久拒絕 | 已接 action／target ID／關鍵字篩選、50 筆分頁與 before/after/details 明細；同館別 manager/admin 可讀 | 篩選表單、時間線卡片與詳細 sheet | **部分完成：即時唯讀投影、v3/v4 欄位相容、角色 Rules 與明細已接；v3 日期範圍篩選與伺服器端查詢／匯出待實作** |
