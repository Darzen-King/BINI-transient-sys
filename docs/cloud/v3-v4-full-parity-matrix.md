...
| 5 | 付款管理 `payments.html` | `/payments`、新增／刪除付款、選在住房客或手動輸入、日結、CSV 匯出 | `payments` 即時讀取、`paymentCreate`／`paymentRefund` callable；後續 `cashierSessions`、`payment.delete`、`cashier.close` 與伺服器匯出 | 已接當日摘要、付款紀錄、在住房一般收款與追加式退款；手動例外與帳務維護待補 | 同一 responsive 收款表單、摘要卡、付款卡與退款 dialog | **部分完成：MFA＋`payments` 權限的一般收款與追加式退款、即時紀錄／摘要、transaction／audit／operation replay 已接；退款會保留原付款並在伺服器重新檢查可退餘額。訂金調整、手動例外、刪除、日結與 CSV 待實作** |
