...
| 7 | 新增預約 `booking_create.html` | `/bookings/new`、`/bookings/multi`、availability、quote、多時段、折扣、付款 | `bookingCreate` callable；MFA＋頁面權限、跨 collection transaction、v3 計價、訂金／audit／operation ID | 單筆完整欄位與 `bookingPreview` 送出前 availability／quote 已接；多時段與費率參考待補 | 單欄觸控表單；與桌機共用 contract／權限／transaction | **部分完成：單筆預約、手動覆寫、自動計價、折扣、訂金與 server-authoritative 前置預覽已接；建立 transaction 仍會重驗，避免 preview race；多時段建立與費率參考待實作** |
