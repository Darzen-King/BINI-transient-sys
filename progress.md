...
### 階段 3：預約管理即時讀取／建立／取消切片
- **狀態：** in_progress（讀取、單筆建立與取消完成；其餘寫入流程未開始）
...
- `bookingCancel` callable 與預約明細 dialog/bottom sheet 已完成：僅具 `bookings` 頁面權限的 MFA 使用者可取消仍為 `已預約` 的文件，transaction 原子更新狀態／version／audit／operation replay；UI 有二次確認且網路重送維持 UUID。v3 No-show 通知、修改、多時段與送出前 quote 仍未實作。
...
| 預約建立／取消切片 | shared quote/conflict、MFA/page access、UI gateway/retry | v3 多日計價、衝突、權限、operation retry、取消確認與品牌 icon 關聯 | 建立／取消聚焦測試通過；完整 128/128；Rules 41/41 | 通過 |
| DEV Functions / Hosting（預約建立／取消切片） | `bini-transient-dev` | `bookingCreate`／`bookingCancel` callable 與最新版 PWA bundle | 10 Functions 均為 asia-east1；兩個 booking callable 均為 Node.js 22／512 MiB；首頁、manifest、favicon、PWA 192 icon 及含兩個 callable 的 live bundle 均 HTTP 200 | 通過 |
